...
css_overrides = """
    <style>
        ...
        .page-header { text-align: center; padding: 4rem 2rem; background: var(--bg-gradient); border-bottom: 1px solid var(--glass-border); }
        ...
        .content-wrapper { padding: 4rem 5%; max-width: 1200px; margin: 0 auto; }
        ...
        /* Corporate Cards */
        .project-vault-card, .blog-card { background: rgba(255,255,255,0.02); border: 1px solid var(--glass-border); border-radius: 12px; transition: transform 0.3s, border-color 0.3s; }
        .project-vault-card:hover, .blog-card:hover { transform: translateY(-5px); border-color: var(--accent); }
    </style>
"""

def extract_cards(html_content, class_name):
    # Extracts all divs with the specified class name using a simple regex or string search
    # Since regex for HTML is tricky, we'll try to find the container and get its inner content.
    match = re.search(r'<div class="container">(.*?)</div>\s*(?:<script|</body>)', html_content, re.DOTALL)
    if match:
        content = match.group(1)
        # Remove the old h2 title
        content = re.sub(r'<h2.*?>.*?</h2>', '', content, flags=re.DOTALL)
        # Fix inline pink styles to accent
        content = content.replace('rgba(236, 72, 153, 0.1)', 'rgba(242, 140, 40, 0.1)')
        content = content.replace('#ec4899', 'var(--accent)')
        return content.strip()
    return ""

def process_page(filename, is_ar, page_title, page_subtitle):
    try:
    ...
    cards_html = extract_cards(raw, "")
    if not cards_html:
        # Fallback if no <div class="container">
        return
...
# Process Projects
process_page('projects.html', False, 'Success Portfolio', 'Proven track record of engineering excellence and cost reduction.')
process_page('projects-ar.html', True, 'سابقة الأعمال', 'سجل حافل بالتميز الهندسي وخفض التكاليف.')

# Process Blog
process_page('blog.html', False, 'Knowledge Hub', 'Insights, case studies, and engineering best practices.')
process_page('blog-ar.html', True, 'مركز المعرفة', 'رؤى، دراسات حالة، وأفضل الممارسات الهندسية.')

# Process Certificates
process_page('certificates.html', False, 'Accreditations', 'Professional certifications and licenses.')
process_page('certificates-ar.html', True, 'الشهادات والاعتمادات', 'الرخص والشهادات المهنية المعتمدة.')

print("Inner pages rebuilt successfully.")
