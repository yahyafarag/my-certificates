...
def extract_content(filename):
    ...
    if "blog" in filename:
        cards = re.findall(r'(<article class="blog-card".*?</article>)', html, flags=re.DOTALL)
        content = "\n".join(cards)
        content = f'<div class="grid-layout">{content}</div>'
    elif "certificates" in filename:
        # Get the certificates section
        ...
        content = content_match.group(1) if content_match else ""
        content = f'<div style="max-width: 800px; margin: 0 auto; margin-top: 80px;">{content}</div>'

    # Fix inline pink to orange
    content = content.replace('rgba(236, 72, 153, 0.1)', 'rgba(242, 140, 40, 0.1)')
    content = content.replace('#ec4899', 'var(--accent)')
    return title, content

def rebuild_file(filename, is_ar, page_title, page_subtitle, is_hub=False):
...
hub_pages = {
    'blog.html': (False, 'Knowledge Hub', 'Insights, case studies, and engineering best practices.'),
    'blog-ar.html': (True, 'مركز المعرفة', 'رؤى، دراسات حالة، وأفضل الممارسات الهندسية.'),
    'certificates.html': (False, 'Accreditations', 'Professional certifications and licenses.'),
    'certificates-ar.html': (True, 'الشهادات والاعتمادات', 'الرخص والشهادات المهنية المعتمدة.')
}

for f, args in hub_pages.items():
    rebuild_file(f, args[0], args[1], args[2], is_hub=True)
...
for a in articles:
    is_ar = '-ar.html' in a
    rebuild_file(a, is_ar, '', '', is_hub=False)
